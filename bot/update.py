...
async def update_name(message: Message, state: FSMContext) -> None:
    ...
    poster_id = poster.update_customer_info(**{"client_id": poster_id, "client_name": new_name})
    customer = poster.get_customer_by_id(poster_id)
...
async def update_phone(message: Message, state: FSMContext) -> None:
    ...
    if new_phone:
        is_exist = poster.get_customer_by_phone(new_phone)
        if is_exist:
        ...
        poster_id = customer.poster_id
        customer_id = customer.id
        poster_id = poster.update_customer_info(**{"client_id": poster_id, "phone": new_phone})
        customer = poster.get_customer_by_id(poster_id)
